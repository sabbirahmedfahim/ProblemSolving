...
void solve()
{
    ...
    priority_queue<pair<ll, pair<ll, ll>>> pq; // {len, hash}
    for (int i = 0; i < s.size() - 1; i++)
    {
        pair<ll, ll> currLeftHash = hash.getPrefixHash(0, i);
        pair<ll, ll> currRightHash = hash.getPrefixHash(s.size() - 1 - i, s.size() - 1);

        if(currLeftHash == currRightHash)
        {
            pq.push({i + 1, {currLeftHash.first, currLeftHash.second}});
        }

        // cerr << 0 << " " << i << " : " << s.size() - 1 - i << " " << s.size() - 1 << nl;
    }
    
    // while (!pq.empty())
    // {
    //     cerr << pq.top().first << " : " << pq.top().second.first << " " << pq.top().second.first << nl;
    //     pq.pop();
    // }

    while (!pq.empty())
    {
        int lenn = pq.top().first;
        pair<ll, ll> needHash = pq.top().second; // pair

        for (int l = 1, r = 1; r < s.size() - 1; r++)
        {
            if(r - l + 1 == lenn)
            {
                pair<ll, ll> currHash = hash.getPrefixHash(l, r);
                if(currHash == needHash)
                {
                    string res;
                    for (int j = l; j <= r; j++) res += s[j];
                    cout << res << nl; return;
                }
