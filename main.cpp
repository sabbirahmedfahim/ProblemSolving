...
struct StringHash{
    ...
    */
    ...
    StringHash(const string&s){
        ...
        base1=bases[rand()%4],base2=bases[rand()%4];while(base2==base1)base2=bases[rand()%4];
        mod1=mods[rand()%4],mod2=mods[rand()%4];while(mod2==mod1)mod2=mods[rand()%4];
    ...
    void buildPrefixHash(const string&s){ // O(n)
        prefixHash[0]={s[0],s[0]};basePower[0]={1,1};
    ...
    pair<ll,ll> concatSubstr(int l1,int r1,int l2,int r2){ // O(1)
        auto hashLeft=getPrefixHash(l1,r1),hashRight=getPrefixHash(l2,r2);
    ...
    void appendChar(char c){ // O(1)
        ...
        ll powFirst=basePower.back().first*base1%mod1,powSecond=basePower.back().second*base2%mod2;
...
void solve(){
    ...
    s.push_back('a');hash.appendChar('a');
}
