...
int main()
{
    ...
    for (int i = 0; i < queries; i++) // O((q + n) * sqrt(n))
    {
        ...
        while (curr_r < r)
        {
            curr_r++;
            p.insert(a[curr_r]);
            curr_ans += p.order_of_key(a[curr_r]);
            // curr_ans += a[curr_r];
        }
        while (curr_l > l)
        {
            curr_l--;
            p.insert(a[curr_l]);
            curr_ans += p.order_of_key(a[curr_l]);
            // curr_ans += a[curr_l];
        }
        while (curr_l < l)
        {
            // curr_ans -= a[curr_l];
            p.erase(p.find_by_order(p.order_of_key(a[curr_l])));
            curr_ans -= p.order_of_key(a[curr_l]);
            curr_l++;
        }
        while (curr_r > r)
        {
            // curr_ans -= a[curr_r];
            p.erase(p.find_by_order(p.order_of_key(a[curr_r])));
            curr_ans -= p.order_of_key(a[curr_r]);
            curr_r--;
        }
