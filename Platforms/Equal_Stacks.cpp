...
int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    ...
    int h1_height = 0, h2_height = 0, h3_height = 0, mx_height = 0;
    for(auto e : h1) h1_height += e;
    for(auto e : h2) h2_height += e;
    for(auto e : h3) h3_height += e;

    while(!h1.empty() || !h2.empty() || !h3.empty())
    {
        if(h1_height == h2_height && h2_height == h3_height) 
        {
            mx_height = h1_height; break;
        }

        if(h1_height >= h2_height && h1_height >= h3_height)
        {
            if(h1.empty()) break;
            h1_height -= h1.back(); h1.pop_back();
        }
        else if(h2_height >= h1_height && h2_height >= h3_height)
        {
            if(h2.empty()) break;
            h2_height -= h2.back(); h2.pop_back();
        }
        else if(h3_height >= h2_height && h3_height >= h1_height)
        {
            if(h3.empty()) break;
            h3_height -= h3.back(); h3.pop_back();
        }
