...
struct StringHash
{
    ...
    */
    ...
    void buildSuffixHash(string &s) // O(n) if called
    {
        int n = s.size();
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int mxLen = min(i - 0, (n - 1) - i);
        // if(mxLen == 0) continue;

        int lo = 0, hi = mxLen, res = 0;
        while (lo <= hi)
        {
            int mid = lo + (hi - lo)/2;
            pair<ll, ll> pref = hash.getPrefixHash(i - mid, i + mid);
            pair<ll, ll> suff = hash.getSuffixHash(i - mid, i + mid);
            if(pref == suff) 
            {
                mx = max(mx, mid);
                lo = mid + 1;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            int mid = lo + (hi - lo)/2;
            pair<ll, ll> pref = hash.getPrefixHash(i - mid - 1, i + mid);
            pair<ll, ll> suff = hash.getSuffixHash(i - mid - 1, i + mid);
            if(pref == suff) 
            {
                mx = max(mx, mid + 1);
                lo = mid + 1;
