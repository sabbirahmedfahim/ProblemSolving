...
int main()
{
    ...
    while (cin >> n)
    {
        ...
        StringHash hash1(s), hash2(ss);
        set<int> res;
    
        // cout << s << " " << ss << nl;
    
        for (int l = 0, r = 0; r < ss.size() && s.size() <= ss.size(); r++)
        {
            if(r - l + 1 == s.size())
            {
                // cerr << "##" << nl;
                pair<ll, ll> pref1 = hash1.getPrefixHash(0, n - 1), 
                pref2 = hash2.getPrefixHash(l, r);
                // if(hash1.getPrefixHash(0, n - 1) == hash2.getPrefixHash(l, r));
                if(pref1 == pref2)
                {
                    res.insert(l);
                    // cerr << l << " -- " << r << " : " << hash1.getPrefixHash(0, n - 1).first << " -- " << hash2.getPrefixHash(l, r).first << nl;
                }
